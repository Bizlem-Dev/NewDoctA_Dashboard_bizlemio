...
@SlingServlet(paths = "/getDocumentScriptDataHash")

public class getDocumentScriptDataHashChanged extends SlingAllMethodsServlet {
	...
	protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response)
		...
		try {
			...
			if (!isNullString(email)) {
			    ...
			    System.out.println("1 ::::: "+formatter.format(checkerdate)); 
				Node userEmailNode = cart.grtServiceidnode(freetrialstatus, email, session, response, lgtype);
				System.out.println("2 ::::: "+formatter.format(checkerdate));
								...
								while (itr.hasNext()) {
									...
									if (nextNodeTemplateName.hasNodes()) {
										...
										while (itr2.hasNext()) {
											...
											//JSONArray documentTrack=callScriptDocument(nextNodeOfTemplateInside, document_url, session, response, GenerationDate);
											//checkerdate = new Date();
											//System.out.println("freetrial 3 ::::: "+formatter.format(checkerdate));
											JSONArray documentTrack=callScriptDocument(nextNodeOfTemplateInside, document_url, session, response, GenerationDate);
											//out.println("documentTrack: "+documentTrack.length());
											//checkerdate = new Date();
											//System.out.println("freetrial 4 ::::: "+formatter.format(checkerdate));
											if(documentTrack.length()!=0 && documentTrack!=null){
												nextNodeOfTemplateInside.setProperty("documentTrackAllArray", documentTrack.toString());
												session.save();
											...
											// mailtracking code start here
											
											/**
											 * here called script for mailtracking api.
											 *  
											 *  */
											
										   JSONArray mailTrack=callScriptMail(nextNodeOfTemplateInside, document_url, session, response);
											
											if(mailTrack.length()>0){
												
												nextNodeOfTemplateInside.setProperty("mailTrackAllArray", mailTrack.toString());
												session.save();
										...
										while (itr.hasNext()) {
											...
											if (nextNodeTemplateName.hasNodes()) {
												...
												while (itr2.hasNext()) {
													...
													/**
													 * here called script for document file.
													 *  
													 *  */
													//checkerdate = new Date();
													//System.out.println("service 3 ::::: "+formatter.format(checkerdate));
													JSONArray documentTrack=callScriptDocument(nextNodeOfTemplateInside, document_url, session, response, GenerationDate);
													//out.println("documentTrack: "+documentTrack.length());
													//checkerdate = new Date();
													//System.out.println("service 4 ::::: "+formatter.format(checkerdate));
													...
													if(mailTrack.length()!=0 && mailTrack!=null){
